...
const upload = multer({ dest: 'stock/' });
...
app.post('/upload', requireLogin, upload.single('file'), (req, res) => {
  ...
  const newPath = `stock/${req.file.originalname}`;
...
app.get('/edit', requireLogin, (req, res) => {
  const totalLines = countLinesInFolder('./stock');
    ...
    if (err) {
      ...
      const stockFileLinks = stockFiles.map((file) => {
        return `<div class="file-item">
              ...
              <button class="rename-button" data-folder="stock" data-file="${file}" onclick="openRenameModal(this)">Rename</button>

                <button class="delete-button" onclick="deleteFile('stock', '${file}')">Delete</button>
              </div>
            </div>`;
      }).join('');
  ...
  background-color: #0056b3;
          ...
          <div class="main">
            ...
            <div class="cardBox">
                <div class="card">
                    <div>
                    <div class="numbers">${totalLines}</div>
    ...
    <div class="fancy-input-container">
      ...
      <button class="fancy-button" type="button" onclick="createFile()">Create</button>
            ...
            <div class="files-section">
              ...
              <h2 class="section-title">Stock Files</h2>
...
app.listen(config.port, '0.0.0.0', () => {
  console.log(`Server started on port ${config.port} (http://0.0.0.0:${config.port})`);
});